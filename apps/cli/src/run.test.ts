...
describe("sprout CLI command surface", () => {
  ...
  test("deploy polls GET /v1/preview after 202 starting until running", async () => {
    let polls = 0;
    const baseUrl = startGateway(async (req, url) => {
      captured.push({
        method: req.method,
        path: url.pathname,
        body: req.method === "POST" ? await req.json() : Object.fromEntries(url.searchParams),
        authorization: req.headers.get("authorization"),
      });
  ...
  test("deploy surfaces health_timeout from status poll", async () => {
    let polls = 0;
    const baseUrl = startGateway(async (req, url) => {
      if (url.pathname === "/v1/deploy") {
        return Response.json(
