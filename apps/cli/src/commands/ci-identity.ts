...
/**
 * Full CI identity for `sprout ci *`: reuses deploy/teardown's resolveIdentity
 * path, then adds pipeline-source policy, image ref, and preview hostname.
 */
export async function resolveCiIdentity(
  ...
  const base = await resolveIdentity(deps);
  if (!base.ok) {
    if (base.error.startsWith("cannot derive pr id")) {
      return {
        ok: false,
        error:
