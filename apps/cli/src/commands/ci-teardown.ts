...
import { publishTeardownNote } from "./forge-note.ts";
...
export async function runCiTeardown(
  ...
  const noted = await publishTeardownNote(ctx.deps, identity);
  if (!noted.ok) {
    ctx.deps.io.stderr(`warning: MR note update failed: ${noted.error}`);
  }
  return 0;
}
