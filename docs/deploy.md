...
2. For HTTPS routers, set Traefik label knobs to match that proxy — e.g.
   `SPROUT_TRAEFIK_ENTRYPOINTS=https` and `SPROUT_TRAEFIK_CERTRESOLVER=letsencrypt`
   on Coolify. Leave certresolver blank to use Traefik's default cert
   (`tls=true` only).
...
Optional, from gateway env (omitted when unset/blank):
...
Plain `tls=true` with no certresolver is valid (Traefik default/builtin cert).
Entrypoint and certresolver names are **not** Coolify-specific — set them to
match your Traefik (e.g. Coolify often uses `https` + `letsencrypt`; stock
Traefik quickstarts often use `websecure` + a custom resolver name).
...
| `SPROUT_TRAEFIK_ENTRYPOINTS` | _(empty — omit label)_ |
| `SPROUT_TRAEFIK_CERTRESOLVER` | _(empty — plain `tls=true`)_ |

For HTTPS behind an external Traefik, set entrypoints/certresolver to that
proxy's names. Example Coolify-shaped values (operator choice, not sprout
defaults): `SPROUT_TRAEFIK_ENTRYPOINTS=https` and
